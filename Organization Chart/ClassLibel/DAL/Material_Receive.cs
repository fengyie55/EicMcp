...
namespace Maticsoft.DAL
{
	...
	public partial class Material_Receive
	{
		...
		public decimal Add(Maticsoft.Model.Material_Receive model)
		{
			...
			strSql.Append("ReceiveID,MaterialNum,ClientNum,Count,UserID,WorkStationID,DataTime)");
			strSql.Append(" values (");
			strSql.Append("@ReceiveID,@MaterialNum,@ClientNum,@Count,@UserID,@WorkStationID,@DataTime)");
			strSql.Append(";select @@IDENTITY");
			SqlParameter[] parameters = {
					new SqlParameter("@ReceiveID", SqlDbType.VarChar,15),
					new SqlParameter("@MaterialNum", SqlDbType.VarChar,30),
					new SqlParameter("@ClientNum", SqlDbType.VarChar,15),
		...
		public Maticsoft.Model.Material_Receive GetModel(decimal Re_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Re_ID,ReceiveID,MaterialNum,ClientNum,Count,UserID,WorkStationID,DataTime from tb_Material_Receive ");
			strSql.Append(" where Re_ID=@Re_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@Re_ID", SqlDbType.Decimal)
			};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Re_ID,ReceiveID,MaterialNum,ClientNum,Count,UserID,WorkStationID,DataTime ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" Re_ID,ReceiveID,MaterialNum,ClientNum,Count,UserID,WorkStationID,DataTime ");
			strSql.Append(" FROM tb_Material_Receive ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
