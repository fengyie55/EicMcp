...
namespace Maticsoft.DAL
{
	...
	public partial class tb_BoxInfo
	{
		...
		public decimal Add(Maticsoft.Model.tb_BoxInfo model)
		{
			...
			parameters[0].Value = model.BoxSn;
		...
		public Maticsoft.Model.tb_BoxInfo GetModel(decimal ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,BoxSn,Qty,Type from tb_BoxInfo ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Decimal)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.tb_BoxInfo DataRowToModel(DataRow row)
		{
			Maticsoft.Model.tb_BoxInfo model=new Maticsoft.Model.tb_BoxInfo();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BoxSn,Qty,Type ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,BoxSn,Qty,Type ");
			strSql.Append(" FROM tb_BoxInfo ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
