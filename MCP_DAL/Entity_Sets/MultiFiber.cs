...
namespace Maticsoft.DAL
{
    ...
    public partial class MultiFiber
    {
        ...
        //连接到MCP数据库
        DbHelperSQL dbs = new DbHelperSQL( Model.E_ConnectionList.TotalScanSQLDB);
        #region  BasicMethod
        ...
        public bool Exists(int ID, string ConnectorID)
        {
            ...
            strSql.Append(" where ID=@ID and ConnectorID=@ConnectorID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4),
					new SqlParameter("@ConnectorID", SqlDbType.NVarChar,255)};
            parameters[0].Value = ID;
            parameters[1].Value = ConnectorID;
        ...
        public bool Delete( string ConnectorID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from tblMultiFiber ");
            strSql.Append(" where ConnectorID=@ConnectorID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ConnectorID", SqlDbType.NVarChar,255)			};
        ...
        public Maticsoft.Model.MultiFiber GetModel( string ConnectorID)
        {
            ...
            Maticsoft.Model.MultiFiber model = new Maticsoft.Model.MultiFiber();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Maticsoft.Model.MultiFiber DataRowToModel(DataRow row)
        {
            Maticsoft.Model.MultiFiber model = new Maticsoft.Model.MultiFiber();
            if (row != null)
            {
        ...
        public DataSet GetList(string _Field, string _Date,string Angle)
        {
            if (_Field != "")
            {
                ...
                strSql.Append(_Field);    //字段
                strSql.Append(" FROM tblMultiFiber ");
                if (_Date.Trim() != "")
                {
                    strSql.Append(" WHERE (CONVERT(varchar(100), ScanTime, 23) = '" + _Date + "') AND (TestPassed = N'Pass') AND (NominalYAngle = '"+Angle+"') ");
                }
                return dbs.Query(strSql.ToString());
            }
            else
            {
                string sql = "  SELECT TOP (125) CONVERT(varchar(100), ScanTime, 23) AS 日期, XEndFaceAngle, YEndFaceAngle, MaxFibDiffAdjH, NEWID() AS Random";
                sql += " FROM tblMultiFiber ";
                sql += " WHERE (CONVERT(varchar(100), ScanTime, 23) = '" + _Date + "') AND (TestPassed = N'Pass') ORDER BY Random";
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            {
                strSql.Append("order by T.ConnectorID desc");
            }
        ...
        {
            string strSql = "SELECT ConnectorID, SUBSTRING(ConnectorID, 12, 2) AS Name, TestPassed , * FROM tblMultiFiber";
            strSql += " WHERE (ConnectorID IN ('" + e.SN + "-1', '" + e.SN + "-2', '" + e.SN + "-3','" + e.SN + "-4')) AND (TestPassed = N'Pass')";                                     
            e.TestData = dbs.Query(strSql);                  //返回查询数据
        ...
        {
            string strSql = "SELECT ConnectorID, SUBSTRING(ConnectorID, 12, 2) AS Name, TestPassed , * FROM tblMultiFiber";
            strSql += " WHERE (ConnectorID IN ('" + SN + "-1', '" + SN + "-2', '" + SN + "-3','" + SN + "-4')) AND (TestPassed = N'Pass')";
            return dbs.Query(strSql);                  //返回查询数据
