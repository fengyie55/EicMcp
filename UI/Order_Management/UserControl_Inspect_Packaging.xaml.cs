...
namespace UI
{
    ...
    /// </summary>
    public partial class UserControl_Inspect_Packaging : DocumentContent
    {
        public UserControl_Inspect_Packaging()
        {
            InitializeComponent();
        }
        ...
        private void InspectSN(string ClientSN, string PigtailSN)
        {
            ...
            if (_WTT_Inspect.InspectStart(_SN))
            {
                ...
                //显示待打印的信息
                if (_WorkOrder.InspectMethod == Maticsoft.Model.E_InspectMethod.两码两签) { _WTT_NotPrintCount = _WTT_NotPrintCount + 2; }
                else { _WTT_NotPrintCount++; }

                if (_WTT_NotPrintCount >= 12) { _WTT_NotPrintCount = 0; }
        ...
        private void txb_BarCode_Search_Order_KeyUp(object sender, KeyEventArgs e)
        {
            if (txb_BarCode_Search_Order.IsFocused && e.Key == Key.Enter)
            {
                ...
                else { My_MessageBox.My_MessageBox_Message("Sorry！\r\n未找到此条码的任何信息！"); txb_BarCode_Search_Order.Text = ""; }
            }
